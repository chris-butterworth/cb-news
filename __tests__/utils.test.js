...
const { getPagination } = require('../utils/app-utils')
...
describe('getPagination', () => {
	test('returns an object containing the limit and offset', () => {
		const testSize = 8
		const testPage = 5

		const actualOutput = getPagination(testPage, testSize)

		expect(actualOutput).toEqual({ limit: 8, offset: 40 })
	})
	test('returns an object containing default values if not passed any data', () => {
		const actualOutput = getPagination()

		expect(actualOutput).toEqual({ limit: 5, offset: 0 })
	})
	test('returns an object containing default values if passed invalid data', () => {
		const actualOutput = getPagination('bananas')
